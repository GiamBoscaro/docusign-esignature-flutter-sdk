...
class Workflow {
  ...
  @override
  String toString() => 'Workflow[currentWorkflowStepId=$currentWorkflowStepId, resumeDate=$resumeDate, scheduledSending=$scheduledSending, workflowStatus=$workflowStatus, workflowSteps=$workflowSteps]';
  ...
  static Map<String, List<Workflow>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Workflow.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
