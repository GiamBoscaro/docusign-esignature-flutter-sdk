...
class Groups {
  ...
  @override
  String toString() => 'Groups[endPosition=$endPosition, groups=$groups, nextUri=$nextUri, previousUri=$previousUri, resultSetSize=$resultSetSize, startPosition=$startPosition, totalSetSize=$totalSetSize]';
  ...
  /// Returns a new [Groups] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Groups? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Groups[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Groups[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Groups>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Groups>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Groups.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Groups> mapFromJson(dynamic json) {
    final map = <String, Groups>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Groups.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Groups-objects as value to a dart map
  static Map<String, List<Groups>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Groups>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Groups.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
