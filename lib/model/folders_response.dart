...
class FoldersResponse {
  ...
  @override
  String toString() => 'FoldersResponse[endPosition=$endPosition, envelopes=$envelopes, folders=$folders, nextUri=$nextUri, previousUri=$previousUri, resultSetSize=$resultSetSize, startPosition=$startPosition, totalSetSize=$totalSetSize]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FoldersResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FoldersResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<FoldersResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FoldersResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
