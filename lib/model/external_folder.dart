...
class ExternalFolder {
  /// Returns a new [ExternalFolder] instance.
  ExternalFolder({
    this.endPosition,
    this.errorDetails,
    this.id,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ExternalFolder &&
  ...
  @override
  String toString() => 'ExternalFolder[endPosition=$endPosition, errorDetails=$errorDetails, id=$id, items=$items, name=$name, nextUri=$nextUri, previousUri=$previousUri, resultSetSize=$resultSetSize, startPosition=$startPosition, totalSetSize=$totalSetSize]';
  ...
  /// Returns a new [ExternalFolder] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ExternalFolder? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ExternalFolder[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ExternalFolder[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ExternalFolder>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ExternalFolder>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ExternalFolder.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ExternalFolder> mapFromJson(dynamic json) {
    final map = <String, ExternalFolder>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ExternalFolder.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ExternalFolder-objects as value to a dart map
  static Map<String, List<ExternalFolder>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ExternalFolder>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ExternalFolder.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
