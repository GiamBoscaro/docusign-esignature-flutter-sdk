...
class BillingInvoice {
  ...
  @override
  String toString() => 'BillingInvoice[amount=$amount, balance=$balance, dueDate=$dueDate, invoiceId=$invoiceId, invoiceItems=$invoiceItems, invoiceNumber=$invoiceNumber, invoiceUri=$invoiceUri, nonTaxableAmount=$nonTaxableAmount, pdfAvailable=$pdfAvailable, taxableAmount=$taxableAmount]';
  ...
  static Map<String, List<BillingInvoice>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BillingInvoice.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
