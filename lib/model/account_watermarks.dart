...
class AccountWatermarks {
  /// Returns a new [AccountWatermarks] instance.
  AccountWatermarks({
    this.displayAngle,
    this.enabled,
    this.font,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AccountWatermarks &&
  ...
  @override
  String toString() => 'AccountWatermarks[displayAngle=$displayAngle, enabled=$enabled, font=$font, fontColor=$fontColor, fontSize=$fontSize, id=$id, imageBase64=$imageBase64, transparency=$transparency, watermarkText=$watermarkText]';
  ...
  /// Returns a new [AccountWatermarks] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AccountWatermarks? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AccountWatermarks[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AccountWatermarks[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AccountWatermarks(
  ...
  static List<AccountWatermarks>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AccountWatermarks>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AccountWatermarks.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AccountWatermarks> mapFromJson(dynamic json) {
    final map = <String, AccountWatermarks>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AccountWatermarks.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AccountWatermarks-objects as value to a dart map
  static Map<String, List<AccountWatermarks>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AccountWatermarks>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AccountWatermarks.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
