...
class Title {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Title &&
  ...
  /// Returns a new [Title] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Title? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Title[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Title[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, Title> mapFromJson(dynamic json) {
    final map = <String, Title>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Title.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Title-objects as value to a dart map
  static Map<String, List<Title>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Title>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Title.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
