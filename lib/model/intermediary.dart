...
class Intermediary {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Intermediary[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Intermediary[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Intermediary>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Intermediary>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Intermediary.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Intermediary> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = Intermediary.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<Intermediary>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Intermediary>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Intermediary.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
