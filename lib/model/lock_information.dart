...
class LockInformation {
  /// Returns a new [LockInformation] instance.
  LockInformation({
    this.errorDetails,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LockInformation &&
  ...
  @override
  String toString() => 'LockInformation[errorDetails=$errorDetails, lockDurationInSeconds=$lockDurationInSeconds, lockedByApp=$lockedByApp, lockedByUser=$lockedByUser, lockedUntilDateTime=$lockedUntilDateTime, lockToken=$lockToken, lockType=$lockType, useScratchPad=$useScratchPad]';
  ...
  /// Returns a new [LockInformation] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static LockInformation? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LockInformation[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LockInformation[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<LockInformation>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <LockInformation>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = LockInformation.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, LockInformation> mapFromJson(dynamic json) {
    final map = <String, LockInformation>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LockInformation.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of LockInformation-objects as value to a dart map
  static Map<String, List<LockInformation>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LockInformation>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = LockInformation.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
