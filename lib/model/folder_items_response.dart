...
class FolderItemsResponse {
  /// Returns a new [FolderItemsResponse] instance.
  FolderItemsResponse({
    this.endPosition,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is FolderItemsResponse &&
  ...
  @override
  String toString() => 'FolderItemsResponse[endPosition=$endPosition, envelopes=$envelopes, folders=$folders, nextUri=$nextUri, previousUri=$previousUri, resultSetSize=$resultSetSize, startPosition=$startPosition, totalSetSize=$totalSetSize]';
  ...
  /// Returns a new [FolderItemsResponse] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static FolderItemsResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FolderItemsResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FolderItemsResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FolderItemsResponse>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FolderItemsResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FolderItemsResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FolderItemsResponse> mapFromJson(dynamic json) {
    final map = <String, FolderItemsResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FolderItemsResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of FolderItemsResponse-objects as value to a dart map
  static Map<String, List<FolderItemsResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FolderItemsResponse>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FolderItemsResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
