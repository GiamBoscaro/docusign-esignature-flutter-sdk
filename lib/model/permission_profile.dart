...
class PermissionProfile {
  /// Returns a new [PermissionProfile] instance.
  PermissionProfile({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PermissionProfile &&
  ...
  @override
  String toString() => 'PermissionProfile[modifiedByUsername=$modifiedByUsername, modifiedDateTime=$modifiedDateTime, permissionProfileId=$permissionProfileId, permissionProfileName=$permissionProfileName, settings=$settings, userCount=$userCount, users=$users]';
  ...
  /// Returns a new [PermissionProfile] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PermissionProfile? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PermissionProfile[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PermissionProfile[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PermissionProfile>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PermissionProfile>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PermissionProfile.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PermissionProfile> mapFromJson(dynamic json) {
    final map = <String, PermissionProfile>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PermissionProfile.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PermissionProfile-objects as value to a dart map
  static Map<String, List<PermissionProfile>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PermissionProfile>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PermissionProfile.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
