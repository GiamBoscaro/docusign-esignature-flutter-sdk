...
class GroupUsers {
  /// Returns a new [GroupUsers] instance.
  GroupUsers({
    this.endPosition,
    this.nextUri,
    this.previousUri,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is GroupUsers &&
  ...
  @override
  String toString() => 'GroupUsers[endPosition=$endPosition, nextUri=$nextUri, previousUri=$previousUri, resultSetSize=$resultSetSize, startPosition=$startPosition, totalSetSize=$totalSetSize, users=$users]';
  ...
  /// Returns a new [GroupUsers] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GroupUsers? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GroupUsers[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GroupUsers[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<GroupUsers>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GroupUsers>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GroupUsers.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GroupUsers> mapFromJson(dynamic json) {
    final map = <String, GroupUsers>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GroupUsers.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of GroupUsers-objects as value to a dart map
  static Map<String, List<GroupUsers>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GroupUsers>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GroupUsers.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
