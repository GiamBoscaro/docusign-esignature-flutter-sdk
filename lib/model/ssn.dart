...
class Ssn {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Ssn &&
  ...
  /// Returns a new [Ssn] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Ssn? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Ssn[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Ssn[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Ssn>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Ssn>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Ssn.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Ssn> mapFromJson(dynamic json) {
    final map = <String, Ssn>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Ssn.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Ssn-objects as value to a dart map
  static Map<String, List<Ssn>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Ssn>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Ssn.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
