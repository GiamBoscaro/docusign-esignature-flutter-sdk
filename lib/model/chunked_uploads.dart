...
class ChunkedUploads {
  /// Returns a new [ChunkedUploads] instance.
  ChunkedUploads({
    this.checksum,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ChunkedUploads &&
  ...
  @override
  String toString() => 'ChunkedUploads[checksum=$checksum, chunkedUploadId=$chunkedUploadId, chunkedUploadParts=$chunkedUploadParts, chunkedUploadUri=$chunkedUploadUri, committed=$committed, expirationDateTime=$expirationDateTime, maxChunkedUploadParts=$maxChunkedUploadParts, maxTotalSize=$maxTotalSize, totalSize=$totalSize]';
  ...
  /// Returns a new [ChunkedUploads] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ChunkedUploads? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ChunkedUploads[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ChunkedUploads[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ChunkedUploads>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ChunkedUploads>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ChunkedUploads.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ChunkedUploads> mapFromJson(dynamic json) {
    final map = <String, ChunkedUploads>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ChunkedUploads.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ChunkedUploads-objects as value to a dart map
  static Map<String, List<ChunkedUploads>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ChunkedUploads>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ChunkedUploads.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
