// Copyright 2022 Giammarco Boscaro. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.17

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:docusign_esignature_flutter_sdk/api.dart';
import 'package:test/test.dart';

// tests for BulkSendingList
void main() {
  // final instance = BulkSendingList();

  group('test BulkSendingList', () {
    // An array of `bulkCopy` objects. Each object represents an instance or copy of an envelope and contains details such as the recipient, custom fields, tabs, and other information.
    // List<BulkSendingCopy> bulkCopies (default value: const [])
    test('to test the property `bulkCopies`', () async {
      // TODO
    });

    // The GUID of the bulk send list.
    // String listId
    test('to test the property `listId`', () async {
      // TODO
    });

    // The name of the bulk send list.
    // String name
    test('to test the property `name`', () async {
      // TODO
    });


  });

}
