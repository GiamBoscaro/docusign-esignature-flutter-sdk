// Copyright 2022 Giammarco Boscaro. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.17

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:docusign_esignature_flutter_sdk/api.dart';
import 'package:test/test.dart';

// tests for BulkProcessingListSummary
void main() {
  // final instance = BulkProcessingListSummary();

  group('test BulkProcessingListSummary', () {
    // 
    // String bulkProcessListId
    test('to test the property `bulkProcessListId`', () async {
      // TODO
    });

    // 
    // String createdByUser
    test('to test the property `createdByUser`', () async {
      // TODO
    });

    // The creation date of the account in UTC timedate format.
    // String createdDate
    test('to test the property `createdDate`', () async {
      // TODO
    });

    // 
    // String name
    test('to test the property `name`', () async {
      // TODO
    });


  });

}
